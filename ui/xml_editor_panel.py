...
class FileLoader(QThread):
    ...
    def run(self):
        try:
            # ensure SSH is alive and SFTP is fresh before reading
            self.session.ssh.ensure_connected()
            sftp = self.session.ssh.sftp
            f = sftp.open(self.session.remote_path(self.filename))
            content = f.read().decode(); f.close()
            xsd = ""
            try:
                xf = sftp.open(self.session.remote_path(self.filename.replace(".xml", ".xsd")))
...
class FileSaver(QThread):
    ...
    def run(self):
        try:
            # reconnect if socket closed between load and save
            self.session.ssh.ensure_connected()
            sftp = self.session.ssh.sftp
            if self.backup_name:
                bf = sftp.open(self.session.remote_path(self.backup_name), "w")
                bf.write(self.xml_data); bf.close()
            f = sftp.open(self.session.remote_path(self.filename), "w")
